// vk_host_texture.cpp: Vulkan offscreen renderer for texture sampling tests.
// Config: 512x512 VkImage + VkSampler (Descriptor Set 0, Binding 0).
// Shader: 9-tap animated UV distortion.
//
// Perf Note:
// GPU: Hardware TMU handles bilinear filtering (zero ALU cost).
// CPU: Avoids heavy 9-tap sampling overhead (8 reads + 4 lerps per tap).
...
int main(int argc, char** argv) {
  ...
  appInfo.pApplicationName = "tex_bench";
  ...
  VkDeviceMemory stageMem;
  {
    ...
    void* ptr;
    VK(vkMapMemory(dev, stageMem, 0, texSize, 0, &ptr));
  ...
  VkDeviceMemory texMem;
  {
    ...
    VkMemoryRequirements mr;
    vkGetImageMemoryRequirements(dev, texImg, &mr);
    VkMemoryAllocateInfo mai{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
    mai.allocationSize = mr.size;
    mai.memoryTypeIndex =
  ...
  VkPipelineViewportStateCreateInfo vs_state{
      VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO};
  vs_state.viewportCount = 1;
  vs_state.pViewports = &vp;
  vs_state.scissorCount = 1;
  vs_state.pScissors = &sc;
  ...
  VkDescriptorImageInfo dii{sampler, texView,
  ...
  // ── Render loop ───────────────────────────────────────────────────────────
  double total_ms = 0.0;
  std::cout << "[" << animName << "] GPU texture test: " << W << "x" << H
            << ", " << NFRAMES << " frames\n";
