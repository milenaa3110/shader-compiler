#!/usr/bin/env bash
# run_rv_host.sh <binary> <label> — run a self-verifying RISC-V host binary.
#
# The host binary does its own checks and returns 0 (pass) / non-zero (fail); this
# wrapper just runs it and propagates the exit code. On riscv64 hardware it is
# executed directly; elsewhere it goes through QEMU. Skips cleanly (exit 0) when
# the emulator or the binary is absent so the gate is a no-op off-target.
