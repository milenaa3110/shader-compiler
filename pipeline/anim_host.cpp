// pipeline/anim_host.cpp — x86 multi-frame animation driver.
// Renders NFRAMES of the animated VS+FS shader, writes PPMs, then
// encodes result/anim_x86.mp4 via ffmpeg.
...
// uTime is a uniform global emitted by the anim shaders
...
int main() {
    ...
    static constexpr int NFRAMES = 60;    // 2 seconds @ 30 fps
    ...
    for (int frame = 0; frame < NFRAMES; ++frame) {
        ...
        char path[128];
        std::snprintf(path, sizeof(path), "result/anim_x86_%03d.ppm", frame);
    ...
    const char* out_mp4 = "result/anim_x86.mp4";
    char cmd[512];
    std::snprintf(cmd, sizeof(cmd),
        "ffmpeg -y -framerate 30 -i result/anim_x86_%%03d.ppm "
